...
public class JobCreateDto {
    ...
    @Size(max = 64, message = "所属服务长度不能超过64个字符")
    @Schema(description = "所属服务名称", requiredMode = Schema.RequiredMode.REQUIRED, example = "mms-base-bc")
    private String serviceName;

    @NotBlank(message = "任务编码不能为空")
    @Size(max = 128, message = "任务编码长度不能超过128个字符")
    @Schema(description = "任务编码（在所属服务下唯一）", requiredMode = Schema.RequiredMode.REQUIRED, example = "daily_report")
    private String jobCode;

    @NotBlank(message = "任务名称不能为空")
    @Size(max = 255, message = "任务名称长度不能超过255个字符")
    @Schema(description = "任务名称", requiredMode = Schema.RequiredMode.REQUIRED, example = "日报任务")
    private String jobName;

    @NotBlank(message = "Cron 表达式不能为空")
    @Size(max = 128, message = "Cron 表达式长度不能超过128个字符")
    @Schema(description = "Cron 表达式", requiredMode = Schema.RequiredMode.REQUIRED, example = "0 0 1 * * ?")
