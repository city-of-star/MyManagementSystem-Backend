-- 创建 mms_dev_core 数据库
CREATE DATABASE IF NOT EXISTS `mms_side_income_dev_core` CHARACTER SET utf8mb4 COLLATE utf8mb4_bin;
...
-- 记账种子：账户（归属 lhy / user_id=2；新用户走全局模板懒加载拷贝）
INSERT IGNORE INTO `finance_account`
(`id`, `user_id`, `name`, `account_type`, `opening_balance`, `account_no`, `note`, `sort_order`, `enabled`, `deleted`, `create_time`, `update_time`)
VALUES
    (1, 2, '微信', 'wechat', 0.00, NULL, '微信零钱/收款', 10, 1, 0, NOW(), NOW()),
    (2, 2, 'QQ', 'qq', 0.00, NULL, 'QQ钱包/红包', 20, 1, 0, NOW(), NOW()),
    (3, 2, '招商卡', 'bank', 0.00, NULL, '工资卡（招商银行）', 30, 1, 0, NOW(), NOW()),
    (4, 2, '公积金', 'housing_fund', 0.00, NULL, '一般不提取，个人+公司缴纳均计入', 40, 1, 0, NOW(), NOW()),
...
(`id`, `user_id`, `name`, `direction`, `icon`, `sort_order`, `enabled`, `is_system`, `deleted`, `create_time`, `update_time`)
VALUES
    (11, 2, '赞赏码', 'income', NULL, 10, 1, 1, 0, NOW(), NOW()),
    (12, 2, '微信转账', 'income', NULL, 20, 1, 1, 0, NOW(), NOW()),
    (13, 2, 'QQ红包', 'income', NULL, 30, 1, 1, 0, NOW(), NOW()),
    (14, 2, '网盘推广', 'income', NULL, 40, 1, 1, 0, NOW(), NOW()),
    (15, 2, '工资', 'income', NULL, 50, 1, 1, 0, NOW(), NOW()),
    (16, 2, '租房补贴', 'income', NULL, 60, 1, 1, 0, NOW(), NOW()),
    (17, 2, '其他收入', 'income', NULL, 70, 1, 1, 0, NOW(), NOW()),
    (18, 2, '电脑补贴', 'income', NULL, 52, 1, 1, 0, NOW(), NOW()),
    (19, 2, '加班费', 'income', NULL, 54, 1, 1, 0, NOW(), NOW()),
    (20, 2, '餐补', 'income', NULL, 56, 1, 1, 0, NOW(), NOW()),
    (35, 2, '公司公积金', 'income', NULL, 58, 1, 1, 0, NOW(), NOW()),
    (36, 2, '公司医保', 'income', NULL, 59, 1, 1, 0, NOW(), NOW());

-- 记账种子：支出分类
INSERT IGNORE INTO `finance_category`
(`id`, `user_id`, `name`, `direction`, `icon`, `sort_order`, `enabled`, `is_system`, `deleted`, `create_time`, `update_time`)
VALUES
    (21, 2, '饭钱', 'expense', NULL, 10, 1, 1, 0, NOW(), NOW()),
    (22, 2, '买药', 'expense', NULL, 20, 1, 1, 0, NOW(), NOW()),
    (23, 2, '话费', 'expense', NULL, 30, 1, 1, 0, NOW(), NOW()),
    (24, 2, 'Cursor登录助手', 'expense', NULL, 40, 1, 1, 0, NOW(), NOW()),
    (25, 2, '房租', 'expense', NULL, 50, 1, 1, 0, NOW(), NOW()),
    (26, 2, '社保其他', 'expense', NULL, 60, 1, 1, 0, NOW(), NOW()),
    (27, 2, '公积金扣款', 'expense', NULL, 70, 1, 1, 0, NOW(), NOW()),
    (28, 2, '大额花费', 'expense', NULL, 80, 1, 1, 0, NOW(), NOW()),
    (29, 2, '其他支出', 'expense', NULL, 90, 1, 1, 0, NOW(), NOW()),
    (37, 2, '个税', 'expense', NULL, 62, 1, 1, 0, NOW(), NOW());

-- 记账种子：快捷模板
INSERT IGNORE INTO `finance_recurring`
(`id`, `user_id`, `name`, `direction`, `amount`, `category_id`, `account_id`, `from_account_id`, `to_account_id`,
 `cycle`, `day_of_month`, `weekday`, `sort_order`, `enabled`, `note`, `deleted`, `create_time`, `update_time`)
VALUES
    (31, 2, '每日饭钱', 'expense', 0.00, 21, 1, NULL, NULL, NULL, NULL, NULL, 10, 1, '快捷模板：点一次记一笔，不会自动扣款；金额可改', 0, NOW(), NOW()),
    (32, 2, '话费', 'expense', 0.00, 23, 1, NULL, NULL, NULL, NULL, NULL, 20, 1, '快捷模板：点一次记一笔，不会自动扣款；金额可改', 0, NOW(), NOW()),
    (33, 2, 'Cursor登录助手', 'expense', 0.00, 24, 3, NULL, NULL, NULL, NULL, NULL, 30, 1, '快捷模板：点一次记一笔，不会自动扣款；金额可改', 0, NOW(), NOW()),
...
VALUES
    ...
    -- 个人记账
    (20, 'finance_account_type', '记账账户类型', 1, 30, '个人记账账户类型', 0, NOW(), NOW()),
    (21, 'finance_txn_type', '记账流水类型', 1, 31, '个人记账流水类型', 0, NOW(), NOW()),
    (22, 'finance_txn_status', '记账流水状态', 1, 32, '个人记账流水入账状态', 0, NOW(), NOW()),
    (23, 'finance_direction', '记账分类方向', 1, 33, '个人记账分类收入/支出方向', 0, NOW(), NOW()),
    (24, 'finance_recurring_direction', '记账模板方向', 1, 34, '个人记账快捷模板方向', 0, NOW(), NOW()),
    (25, 'finance_fund_category', '基金分类', 1, 35, '个人记账基金持仓分类', 0, NOW(), NOW()),
    (26, 'finance_fund_quote_status', '基金估值状态', 1, 36, '确认/滞后', 0, NOW(), NOW());
...
VALUES
    ...
    -- 记账账户类型
    (67, 20, '现金', 'cash', 1, 0, 1, '现金账户', 0, NOW(), NOW()),
    (68, 20, '微信', 'wechat', 2, 0, 1, '微信钱包', 0, NOW(), NOW()),
    (69, 20, 'QQ', 'qq', 3, 0, 1, 'QQ钱包', 0, NOW(), NOW()),
    (70, 20, '银行卡', 'bank', 4, 0, 1, '银行卡', 0, NOW(), NOW()),
    (71, 20, '支付宝', 'alipay', 5, 1, 1, '支付宝/余额宝等', 0, NOW(), NOW()),
    (72, 20, '公积金', 'housing_fund', 6, 0, 1, '住房公积金', 0, NOW(), NOW()),
    (73, 20, '社保', 'social_security', 7, 0, 1, '社保账户', 0, NOW(), NOW()),
    (74, 20, '公司卡', 'company_card', 8, 0, 1, '公司卡', 0, NOW(), NOW()),
    (75, 20, '医保', 'medical', 9, 0, 1, '医保账户', 0, NOW(), NOW()),
    (76, 20, '基金', 'fund', 10, 0, 1, '基金持仓', 0, NOW(), NOW()),
    (77, 20, '其他', 'other', 99, 0, 1, '其他账户', 0, NOW(), NOW()),
    -- 记账流水类型
    (78, 21, '收入', 'income', 1, 0, 1, '收入流水', 0, NOW(), NOW()),
    (79, 21, '支出', 'expense', 2, 1, 1, '支出流水', 0, NOW(), NOW()),
    (80, 21, '转账', 'transfer', 3, 0, 1, '账户间转账', 0, NOW(), NOW()),
    (81, 21, '平账', 'adjustment', 4, 0, 1, '余额对齐差额', 0, NOW(), NOW()),
    -- 记账流水状态
    (82, 22, '已入账', 'settled', 1, 1, 1, '已结算入账', 0, NOW(), NOW()),
    (83, 22, '待结算', 'pending', 2, 0, 1, '待结算（收入或转账）', 0, NOW(), NOW()),
    -- 记账分类方向
    (84, 23, '收入', 'income', 1, 0, 1, '收入分类', 0, NOW(), NOW()),
    (85, 23, '支出', 'expense', 2, 1, 1, '支出分类', 0, NOW(), NOW()),
    -- 记账模板方向
    (86, 24, '收入', 'income', 1, 0, 1, '收入模板', 0, NOW(), NOW()),
    (87, 24, '支出', 'expense', 2, 1, 1, '支出模板', 0, NOW(), NOW()),
    (88, 24, '转账', 'transfer', 3, 0, 1, '转账模板', 0, NOW(), NOW()),
    -- 基金分类
    (90, 25, '债基', 'bond', 1, 0, 1, NULL, 0, NOW(), NOW()),
    (91, 25, '主题基金', 'theme', 2, 0, 1, NULL, 0, NOW(), NOW()),
    (92, 25, '指数（国内）', 'index_cn', 3, 1, 1, NULL, 0, NOW(), NOW()),
    (93, 25, '指数（海外）', 'index_overseas', 4, 0, 1, NULL, 0, NOW(), NOW()),
    (94, 25, '美股场外', 'us_otc', 5, 0, 1, NULL, 0, NOW(), NOW()),
    (95, 25, '其他', 'other', 99, 0, 1, NULL, 0, NOW(), NOW()),
    -- 基金估值状态
    (96, 26, '已确认', 'confirmed', 1, 1, 1, '计入总资产', 0, NOW(), NOW()),
    (97, 26, '滞后待更新', 'delayed', 2, 0, 1, '不计入总资产，单独展示', 0, NOW(), NOW());
-- 初始化定时任务数据
