...
2. 投递 MQ（或落 `msg_sys_announce_task` 由作业拉取）→ 消费者分页解析目标用户，批量插入 `msg_sys_inbox`（每用户一行）。此过程技术上称**扇出**（fan-out：一条公告展开成多人收件），界面不展示「扇出」二字。
3. 每批成功后推进游标；全部完成标 `DONE`；失败可重试且按 `(announce_id, user_id)` 唯一键幂等。
4. 在线用户：发送过程中可按批推 WebSocket 未读增量；离线用户登录拉未读即可。

小范围（指定少数人）可同步写入收件箱；全员 / 大角色强制走异步，避免接口超时。
...
| 公告管理 | **进 V1**：CRUD 列表（无操作列，点标题进详情）；已读/未读名单搜索+分页；管理员与超管可发可看；界面状态用「发送中」等；**不做**一键催读 |

### 仍可开工时再定（非产品争议）

- 正式 SQL 迁移脚本文件名与菜单权限种子数据
- 公告失败是否提供「一键重试发送」按钮（建议 V1 做简单重试）
- 是否正式立项开工（文档头仍写「落地待定」）
