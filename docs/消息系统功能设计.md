...
> 状态：**V1 已落地**（2026-08-04）；**V1.1 跳转路径**（2026-08-06，`link_path` + 公告菜单选择）  
...
- 业务页「一键跳转」链接（已支持：业务通知 `linkPath`；公告发件可选菜单跳转，扇出到收件箱）
...
> 说明：业务跳转字段已加回（`link_path`）；用户管理发消息仍不做。F11 现用于公告管理。  
...
| 编号 | 功能 |
|------|------|
| F18 | 系统通知可选 `link_path`（站内相对路径）；详情「去看看」跳转 |
| F19 | 业务通知 Feign 可传 `linkPath`（如记账提醒 → `/finance/recurrings`） |
| F20 | 公告发件/修改可选跳转菜单；`GET /msg/announce/link-options`；修改后同步收件箱 |
...
msg_sys_announce
  ...
  link_path         varchar(200) null -- 可选站内跳转路径
...
msg_sys_inbox
  ...
  biz_id            varchar(64) null  -- 业务侧关联（幂等键；公告为 announceId）
  title             varchar(200)
  content_html      mediumtext null   -- 公告用；业务通知可空
  content_text      varchar(2000)     -- 纯文本或摘要
  link_path         varchar(200) null -- 可选站内跳转（业务通知/公告扇出写入）
...
> `biz_id` 仅幂等，不充当跳转；跳转统一用 `link_path`。
