...
> 状态：已立项 / 实现中（2026-08-03）  
> 仓库：本体 `MyManagementSystem-Backend`（`mms-usercenter-bc`）。  
...
- **菜单归属（收件箱）**：独立顶层菜单（与「首页」同级），**不要**挂在「系统管理」下；权限码建议 `MESSAGE`
- **发公告快捷入口**：消息中心操作行「发系统公告」**保留**（与公告管理页共用同一套发公告能力）

### 4.1.1 公告管理（系统管理子菜单 · V1）

挂在 **系统管理 → 公告管理**（不是消息中心的一部分）。  
面向：**管理员 / 超级管理员**（权限码 `MESSAGE_ANNOUNCE`，按权限不按写死角色名）。

| 能力 | 说明 |
|------|------|
| 公告列表 | 对齐系统管理 CRUD：查询区 + 操作栏 + 表格（含序号、无「操作」列）+ 分页；点**标题**进入详情 |
| 公告详情 | **同页切换**（非抽屉）：左上角「返回」；已读/未读名单支持**搜索 + 分页 + 区内滚动** |
| 公告状态文案 | 界面：待发送 / 发送中 / 已完成 / 失败（库内仍用 PENDING/RUNNING/DONE/FAILED；「扇出」仅技术用语，界面不出现） |
| 发公告 | 本页可发；弹窗与消息中心「发系统公告」**同一套**（宽弹窗 + Quill） |
| 一键提醒未读 | **不做**（V1） |
...
- **业务自动发**：作业失败、关键安全事件 → 写系统通知给相关人（纯文本模板）
...
| 能力 | 谁 |
|------|-----|
| 看自己的收件箱、找人开会话、回私信、标已读、不显示 / 删自己侧会话、收藏系统通知 | 登录用户 |
| 发系统公告、公告管理（列表 / 已读统计） | 管理员 / 超级管理员（持 `MESSAGE_ANNOUNCE`） |
| 冒充系统发「系统通知」 | 禁止普通用户 |
...
- `MESSAGE_ANNOUNCE`（发公告 + 公告管理菜单；管理员与超级管理员均授予）
...
| F11 | **公告管理**（系统管理子菜单：列表、已读/未读统计与名单；本页可发公告） |
...
- **身份**：每人只看自己的信 / 自己的会话成员态；公告管理按 `MESSAGE_ANNOUNCE` 看全部发件记录
- **实时**：复用已有 App 级 WebSocket，只推「未读数 / 有新消息」；完整内容打开收件箱再拉
- **富文本**：前端复用 `QuillEditor`；后端存净化后的 HTML
...
| `消息系统页面原型-公告管理.html` | 系统管理 · 公告管理 | CRUD 列表（对齐 DataTable：序号/居中/border/stripe；无操作列）；点标题进同页详情+返回；发公告弹窗与消息中心一致 |
...
### 9.1 公告按角色 / 全员 → 每人一条（写库与异步）

**方案：发件记录 + 同进程异步扇出（V1）**

1. 管理员提交公告 → 先写 `msg_sys_announce`（状态 `PENDING` / `RUNNING` / `DONE` / `FAILED`；**界面文案**对应：待发送 / 发送中 / 已完成 / 失败）。范围快照写入 JSON（全员标记 / 角色 ID 列表 / 用户 ID 列表）。
2. 目标人数 ≤50：**同步**写入 `msg_sys_inbox`；更大范围：事务提交后由 `schedulerTaskExecutor` 异步分页扇出（按 `(announce_id, user_id)` 幂等）。本地 MQ 为 NoOp 会丢消息，V1 **不以 MQ 为主通道**。
3. 每批成功后推进游标；全部完成标 `DONE`；失败可重试。
4. 在线用户：发送过程中按批推 WebSocket 未读增量；离线用户登录拉未读即可。

小范围（指定少数人）可同步写入收件箱；全员 / 大角色走异步，避免接口超时。
...
- 事件示例：`{ type: "MSG_UNREAD", total, sysUnread, privUnread }` 或更粗的 `MSG_CHANGED`。
- 客户端收到后更新铃铛；打开中心 / 详情再走 HTTP。
...
msg_sys_announce
  ...
  status            tinyint           -- 0待发送 1发送中 2已完成 3失败（界面文案；技术过程称扇出）
...
- `uk_announce_user (announce_id, user_id)`（`announce_id` 非空时幂等；业务通知可不走此唯一，或用 `uk_biz_user (biz_type, biz_id, user_id)`）
...
| 公告管理 | **进 V1**：CRUD 列表（无操作列，点标题进详情）；已读/未读名单搜索+分页；管理员与超管可发可看；界面状态用「发送中」等；**不做**一键催读；失败可「重试发送」 |
| 扇出实现 | V1 同进程异步（`schedulerTaskExecutor`）；≤50 同步 |

### 仍可后续再定

- 多实例部署时是否改回 MQ 主通道扇出
- 正式环境执行增量脚本：`mysql/prod/20260803_2300_msg_inbox_and_permission.sql`（本地库可将 `USE` 改为 `mms_dev_core`）
