...
| 列表操作 | **修改** / **撤回** / **删除** / **重试发送**（按行状态与权限显隐；**待发送/发送中不可改、不可撤**；发送中不可删） |
| 公告详情 | **同页切换**（非抽屉）：左上角「返回」；已读/未读名单支持**搜索 + 分页 + 区内滚动**；已撤回展示提示条 |
| 公告状态文案 | 界面：待发送 / 发送中 / 已完成 / 失败 / **已撤回**（库值 `0~4`；「扇出」仅技术用语，界面不出现） |
| 发公告 / 修改 | 本页可发；弹窗与消息中心「发系统公告」**同一套**（宽弹窗 + Quill）。修改只改标题与正文，**不改发送范围**；同步更新已扇出 inbox，**重置为未读**并推未读校准；仅已完成/失败可改 |
| 撤回 | 仅 **已完成 / 失败** 可撤；软删收件箱，状态置 **已撤回(4)**；用户消息中心不再显示 |
| 删除 | 发件记录逻辑删除 + 收件箱软删；发送中不可删 |
| 重试发送 | 仅 **失败(3)** 可重试 |
| 一键提醒未读 | **不做**（V1） |
...
| 公告管理（列表 / 详情 / 已读统计）与发 / 改 / 撤 / 删 / 重试 | 持对应 `MESSAGE_ANNOUNCE_*` 按钮权限 |
...
| `消息系统页面原型-公告管理.html` | 系统管理 · 公告管理 | CRUD 列表（含操作列：改/撤/删/重试）；状态含已撤回；点标题进同页详情+返回；发公告弹窗与消息中心一致 |
...
1. 管理员提交公告 → 先写 `msg_sys_announce`（状态 `0 PENDING` / `1 RUNNING` / `2 DONE` / `3 FAILED` / `4 RECALLED`；**界面文案**对应：待发送 / 发送中 / 已完成 / 失败 / 已撤回）。范围快照写入 JSON（全员标记 / 角色 ID 列表 / 用户 ID 列表）。
2. **事务提交后**再扇出并推 WS（避免客户端脏读）：目标人数 ≤50 同步写 `msg_sys_inbox`；更大范围由 `schedulerTaskExecutor` 异步分页扇出（按 `(announce_id, user_id)` 幂等）。本地 MQ 为 NoOp 会丢消息，V1 **不以 MQ 为主通道**。
3. 每批成功后推进游标；全部完成标 `DONE`；失败可重试；**撤回**软删 inbox 并标 `RECALLED`。
4. 在线用户：发送过程中按批推 WebSocket 未读校准；离线用户登录拉未读即可。扇出目标**跳过禁用用户**。
...
| 公告管理 | **进 V1**：列表含操作列（改/撤/删/重试）；点标题进详情；已读/未读名单搜索+分页；细权限码；状态含「已撤回」；**不做**一键催读 |
| WS 事件 | `msg_unread`（小写），只推未读聚合 |
| 扇出实现 | 提交后扇出；≤50 同步、更大 `schedulerTaskExecutor` 异步；跳过禁用用户 |

### 仍可后续再定

- 多实例部署时是否改回 MQ 主通道扇出
- 业务自动系统通知（F13～F14）默认模板与接入点
